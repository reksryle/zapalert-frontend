...
const AllUsers = () => {
  ...
  const fetchAllUsers = async () => {
    try {
      const res = await axios.get(`${import.meta.env.VITE_BACKEND_URL}/api/auth/all-users`);
              ...
              {selectedTab === "Valid ID" && (
                <div className="mt-4 flex justify-center">
                  {selectedUser.idImagePath ? (
                    <a
                      href={`${import.meta.env.VITE_BACKEND_URL}/${selectedUser.idImagePath.replace(/\\/g, "/")}`}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="relative group"
                    >
                      <img
                        src={`${import.meta.env.VITE_BACKEND_URL}/${selectedUser.idImagePath.replace(/\\/g, "/")}`}
                        alt="Valid ID"
