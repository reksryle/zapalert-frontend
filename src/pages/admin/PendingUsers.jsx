...
const PendingUsers = () => {
  ...
  const fetchPending = async () => {
    try {
      const res = await axios.get(`${import.meta.env.VITE_BACKEND_URL}/api/auth/pending-users`);
  ...
  const approveUser = async (id) => {
    try {
      await axios.patch(`${import.meta.env.VITE_BACKEND_URL}/api/auth/approve/${id}`);
  ...
  const rejectUser = async (id) => {
    try {
      await axios.delete(`${import.meta.env.VITE_BACKEND_URL}/api/auth/reject/${id}`);
            ...
            >
              ...
              <div>
                ...
                <div className="text-sm text-gray-700 space-y-1 mt-1">
                  ...
                  <a
                    href={`${import.meta.env.VITE_BACKEND_URL}/${user.idImagePath.replace(/\\/g, "/")}`}
                    target="_blank"
              ...
              <div className="flex flex-col items-center gap-2 min-w-[9rem]">
                <img
                  src={`${import.meta.env.VITE_BACKEND_URL}/${user.idImagePath.replace(/\\/g, "/")}`}
                  alt="Valid ID"
                  className="w-32 h-20 object-cover border rounded-md cursor-pointer hover:scale-105 transition-transform"
                  onClick={() => setSelectedImage(`${import.meta.env.VITE_BACKEND_URL}/${user.idImagePath.replace(/\\/g, "/")}`)}
                />
                <button
